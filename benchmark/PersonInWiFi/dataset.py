...
class CSI2MaskPoseDataset(Dataset):
    def __init__(self,
    ...
    def _get_csi(self, csi_path: str):
        ...
        # pha = torch.where(pha == 0, x=torch.tensor(1e-6, dtype=torch.float32), other=pha)  # Avoid division by zero
        # csi = amp / pha
        # csi = self._normalize(csi)
    ...
    def _get_pose(self, pose_path):
        ...
        :param pose_path: the path of the pose npy file

        Return:
        paf: the part affinity fields
        jhm: the joint heatmaps
        """
        pose = np.load(pose_path)
        pose = np.transpose(pose, (1, 2, 0))
        jhm = pose[:, :, :26]
        paf = pose[:, :, 26:]
        jhm = self.transform(jhm).float()
        paf = self.transform(paf).float()

        return jhm, paf

    def __getitem__(self, index):
        csi_path = self.data_list[index]
        csi = self._get_csi(csi_path)
        ...
        # pose
        pose_path = csi_path.replace('npy', '2d_pose').replace('.npz', '.npy')
        jhm, paf = self._get_pose(pose_path)

        return csi, mask, jhm, paf

    def __len__(self):
        return len(self.data_list)

class CSI2MaskPoseDataModule(LightningDataModule):
    ...
    def setup(self, stage):
        if stage == 'fit':
            self.train_dataset = CSI2MaskPoseDataset(
                json_path=self.train_json_path,
                data_root=self.data_root, 
                mode='train'
            )
            self.val_dataset_seen = CSI2MaskPoseDataset(
                json_path=self.train_json_path,
                data_root=self.data_root,
                mode='val'
            )
            self.val_dataset_unseen = CSI2MaskPoseDataset(
                json_path=self.val_json_path,
                data_root=self.data_root,
                mode='val'
            )
        elif stage == 'test':
            self.test_dataset = CSI2MaskPoseDataset(
...
if __name__ == '__main__':
    json_path = 'datas/NYCU/train&val.json'
    dataset = CSI2MaskPoseDataset(json_path=json_path, data_root='/root/bindingvolume/CSI_dataset_NYCU', mode='train')
    for i in range(len(dataset)):
        csi, mask, jhm, paf = dataset[i]
        print(f'csi shape: {csi.shape}, mask shape: {mask.shape}, jhm shape: {jhm.shape}, paf shape: {paf.shape}')

...
class PAFDecoder(nn.Module):
    def __init__(self, in_channels=3, out_channels=52, channel_list=[32, 64, 128, 256, 512]):
        super().__init__()
...
class SM_JHMDecoder(PAFDecoder):
    def __init__(self, in_channels=3, out_channels=1, out2_channels=25, channel_list=[32, 64, 128, 256, 512]):
        super().__init__(in_channels=3, out_channels=out_channels, channel_list=[32, 64, 128, 256, 512])
        self.final2 = nn.Sequential(
            nn.Conv2d(channel_list[0], out2_channels, kernel_size=1),
            # nn.Sigmoid()  # Assuming binary segmentation
        )
...
class PIF(nn.Module):
    def __init__(self, in_channels=103275, reduced_channels=512):
        ...
        self.sm_jhm_decoder = SM_JHMDecoder()
        self.paf_decoder = PAFDecoder()

    def forward(self, x):
        x = self.reducer(x)             # [B, 512, 3, 2]
        x = self.encoder(x)      # x: encoded features, skips: for U-Net
        paf = self.paf_decoder(x)
        sm, jhm = self.sm_jhm_decoder(x)
        return sm, jhm, paf
...
if __name__ == "__main__":
    ...
    sm, jhm, paf = model(dummy_input)
    print(f"SM shape: {sm.shape}, JHM shape: {jhm.shape}, PAF shape: {paf.shape}")
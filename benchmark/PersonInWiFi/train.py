...
from dataset import CSI2MaskPoseDataModule
...
def heatmap2grid(x):
    y = einops.rearrange(x[0:1, :12], 'b c h w -> c b h w')
    return y
...
class WiSegUNetLightning(LightningModule):
    def __init__(self, configs):
        ...
        self.model = PIF(in_channels=length * in_channels, reduced_channels=model_configs['latent_dim'])
        
        # Loss functions
        loss_configs = configs['Loss']
        self.BCE = nn.BCEWithLogitsLoss(reduction='mean')
        self.JHMLoss = nn.MSELoss()
        self.PAFLoss = nn.MSELoss()
        # JHM_configs = loss_configs['JHM']
        # self.JHMLoss = loss.MWLoss(k=JHM_configs['k'], b=JHM_configs['b'])
        # PAF_configs = loss_configs['PAF']
        # self.PAFLoss = loss.MWLoss(k=PAF_configs['k'], b=PAF_configs['b'])
    ...
    def forward(self, csi):
        ...
        sm, jhm, paf = self.model(csi)  # Forward pass through the model

        return sm, jhm, paf


    def training_step(self, batch, batch_idx):
        csi, mask, jhm, paf = batch
        seg, p_jhm, p_paf = self.forward(csi)

        bce_loss = self.BCE(seg, mask)
        jhm_loss = self.JHMLoss(p_jhm, jhm[:, :-1])
        paf_loss = self.PAFLoss(p_paf, paf)

        total_loss = 0.1*bce_loss + jhm_loss + paf_loss

        self.log_dict({
            'train/bce_loss': bce_loss,
            'train/jhm_loss': jhm_loss,
            'train/paf_loss': paf_loss,
        ...
        if batch_idx % 1000 == 0:
            ...
            self.logger.experiment.add_images('train/jhm', torchvision.utils.make_grid(heatmap2grid(jhm), nrow=6), self.global_step, dataformats="CHW")
            self.logger.experiment.add_images('train/p_jhm', torchvision.utils.make_grid(heatmap2grid(p_jhm), nrow=6), self.global_step, dataformats="CHW")
            self.logger.experiment.add_images('train/paf', torchvision.utils.make_grid(heatmap2grid(paf), nrow=6), self.global_step, dataformats="CHW")
            self.logger.experiment.add_images('train/p_paf', torchvision.utils.make_grid(heatmap2grid(p_paf), nrow=6), self.global_step, dataformats="CHW")
...
def main(args):
    ...
    # Setup data module
    dm = CSI2MaskPoseDataModule(configs=configs)

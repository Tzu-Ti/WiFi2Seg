...
class CSI2MaskDataModule(LightningDataModule):
    ...
    def setup(self, stage):
        if stage == 'fit':
            self.train_dataset = CSI2MaskDataset(
                json_path=self.train_json_path,
                data_root=self.data_root, 
                mode='train'
            )
            self.val_dataset_seen = CSI2MaskDataset(
                json_path=self.train_json_path,
                data_root=self.data_root,
                mode='val'
            )
            self.val_dataset_unseen = CSI2MaskDataset(
                json_path=self.val_json_path,
                data_root=self.data_root,
                mode='val'
            )
        elif stage == 'test':
            self.test_dataset = CSI2MaskDataset(
